...
def coverage(c):
    ...
    """
    c.run("coverage run --source wagtail-openedx runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/wagtail-openedx.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ wagtail_openedx")
...
def lint(c):
    ...
    """
    c.run("flake8 wagtail-openedx tests")
...
def release(c, bumpsize=""):
    ...
    c.run(
        'git tag -a {version} -m "New version: {version}"'.format(
            version=wagtail_openedx.__version__
        )
    )
    c.run("git push --tags")
